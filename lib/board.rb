...
class Board
  ...
  def win(row, column, token)
    win_vertical(row, column, token) ||
      win_horizontal(row, column, token) ||
      win_diagonal_left(row, column, token) ||
      win_diagonal_right(row, column, token)
  end
end
